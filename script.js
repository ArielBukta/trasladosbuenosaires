...
const englishCopy = {
  ...
  "Ya sea una llegada a Ezeiza, una agenda corporativa o un viaje al interior, Translados Buenos Aires coordina el recorrido con puntualidad, seguridad y comunicaci\u00f3n clara.": "Whether it is an arrival at Ezeiza, a corporate schedule or a trip beyond the city, Translados Buenos Aires coordinates every journey with punctuality, safety and clear communication.",
  ...
  "CONFIAN EN TRANSLADOS BUENOS AIRES": "COMPANIES THAT TRUST TRANSLADOS BUENOS AIRES", "Empresas que se mueven": "Companies that move", "con nosotros.": "with us.", "Una red que se mueve con precisi\u00f3n.": "A network that moves with precision.",
...
const translatableAttributes = {
  ...
  "BAIC BJ30 de Translados Buenos Aires": "BAIC BJ30 by Translados Buenos Aires",
...
function setLanguage(language) {
  ...
  document.title = active === 'en' ? 'Buenos Aires Transfers | Airport and private chauffeur' : 'Translados Buenos Aires | Ezeiza, Aeroparque y chofer privado';
  languageButtons.forEach((button) => {
    const selected = button.dataset.language === active;
    button.classList.toggle('active', selected);
    button.setAttribute('aria-pressed', String(selected));
  });
  try { localStorage.setItem('tba-language', active); } catch (_) { /* Storage may be unavailable. */ }
}

languageButtons.forEach((button) => button.addEventListener('click', () => setLanguage(button.dataset.language)));
try { setLanguage(localStorage.getItem('tba-language') || 'es'); } catch (_) { setLanguage('es'); }
